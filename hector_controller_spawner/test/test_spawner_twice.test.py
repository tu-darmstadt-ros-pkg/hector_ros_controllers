...
def generate_test_description():
    ...
    spawner_config = os.path.join(pkg_share, 'test', 'config', 'controller_spawner.yaml')
...
class TestControllerSpawnerIdempotency(unittest.TestCase):
    ...
    def test_spawner_run_twice_idempotent(self):
        ...
        # Spawner command
        cmd = [
            'ros2', 'run', 'hector_controller_spawner', 'hector_controller_spawner',
            '--ros-args', '--params-file', spawner_config
        ]
        ...
        self.assertEqual(return_code1, 0, f"First spawner run failed with return code {return_code1}")
        ...
        self.node.get_logger().info(f"Controller count after first run: {first_run_count}")

        self.assertGreater(first_run_count, initial_count, "Controllers should be loaded after first spawner run")

        # Store the state for comparison
        first_run_active = [c.name for c in first_run_controllers if c.state == 'active']
        first_run_inactive = [c.name for c in first_run_controllers if c.state == 'inactive']
        ...
        self.assertEqual(return_code2, 0, f"Second spawner run failed with return code {return_code2}")
        ...
        self.node.get_logger().info(f"Controller count after second run: {second_run_count}")

        second_run_active = [c.name for c in second_run_controllers if c.state == 'active']
        second_run_inactive = [c.name for c in second_run_controllers if c.state == 'inactive']
        ...
        self.assertEqual(set(first_run_active), set(second_run_active),
                         "Active controllers should be identical after second run")

        # Check that inactive controllers are the same
        self.assertEqual(set(first_run_inactive), set(second_run_inactive),
        ...
        for controller in expected_active:
            self.assertIn(controller, second_run_active,
                          f"Controller {controller} should still be active after second run")

        # Log the outputs for debugging if needed
        if stdout1:
            self.node.get_logger().info(f"First spawner stdout: {stdout1.decode()[:200]}...")
        if stdout2:
            self.node.get_logger().info(f"Second spawner stdout: {stdout2.decode()[:200]}...")

...
from controller_manager_msgs.srv import ListControllers, ListHardwareComponents, SwitchController, SetHardwareComponentState
...
def generate_test_description():
    ...
    spawner_config = os.path.join(pkg_share, 'test', 'config', 'controller_spawner_with_estop.yaml')
...
class TestEStopFunctionality(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.estop_pub = cls.node.create_publisher(
            Bool, 'estop_board/hard_estop', 10
        )
    ...
    def test_complete_estop_workflow(self):
        ...
        self.node.get_logger().info("Step 2: Checking that hardware/controllers are not active")
        ...
        active_hardware = [hw.name for hw in hardware_components if hw.state.label == 'active']
        self.assertEqual(len(active_hardware), 0,
        ...
        self.assertEqual(len(active_controllers), 0,
        ...
        self.node.get_logger().info("Step 4: Checking that hardware/controllers are active")
        hardware_components = self._get_hardware_components()
        controllers = self._get_controller_list()
        ...
        for hw_name in expected_hardware:
            self.assertIn(hw_name, active_hardware,
        ...
        self.node.get_logger().info("Step 5: Activating e-stop again (should deactivate hardware)")
        self._publish_estop(True)
        ...
        active_hardware_after_estop = [hw.name for hw in hardware_components if hw.state.label == 'active']
        self.assertEqual(len(active_hardware_after_estop), 0,
                         "Hardware interfaces should be deactivated when e-stop is activated again")

        # Check that controllers are no longer active
        active_controllers_after_estop = [c.name for c in controllers if c.state == 'active']
        self.assertEqual(len(active_controllers_after_estop), 0,
        ...
        self.node.get_logger().info("Step 7: Checking final state - should be active again")
        ...
        final_active_hardware = [hw.name for hw in final_hardware_components if hw.state.label == 'active']
        for hw_name in expected_hardware:
            self.assertIn(hw_name, final_active_hardware,
                          f"Hardware interface {hw_name} should be active again after second e-stop release")

        # Verify controllers are active again
        final_active_controllers = [c.name for c in final_controllers if c.state == 'active']
        for controller in expected_active_controllers:
            self.assertIn(controller, final_active_controllers,
    ...
    def _get_hardware_components(self):
        ...
        client = self.node.create_client(ListHardwareComponents, '/controller_manager/list_hardware_components')
    ...
    def _unload_hardware_interfaces(self):
        ...
        if not set_hw_state_client.wait_for_service(timeout_sec=5.0):
            self.node.get_logger().error("Hardware component state service not available")
            return

        # Deactivate each active hardware interface
        for hw_component in active_hardware:
            self.node.get_logger().info(f"Deactivating hardware interface: {hw_component}")

            request = SetHardwareComponentState.Request()
            request.name = hw_component
            request.target_state = State()
            request.target_state.id = State.PRIMARY_STATE_INACTIVE  # Transition to inactive
            ...
            if response and response.ok:
                self.node.get_logger().info(f"Successfully deactivated hardware interface: {hw_component}")
            else:
                self.node.get_logger().error(f"Failed to deactivate hardware interface: {hw_component}")
    ...
    def _deactivate_all_controllers(self):
        ...
        request.deactivate_controllers = controllers_to_deactivate  # Controllers to deactivate
        request.strictness = SwitchController.Request.BEST_EFFORT  # Strict mode - fail if any controller can't be switched

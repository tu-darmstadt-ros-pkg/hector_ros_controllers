                                        ...
                                        const double default_velocity_limit )
{
  ...
  for ( size_t i = 0; i < joint_names.size(); ++i ) {
    ...
    case urdf::Joint::CONTINUOUS:
      ...
      joint.velocity_limit = ( urdf_joint->limits ) ? urdf_joint->limits->velocity
                                                    : std::numeric_limits<double>::max();
      break;
    case urdf::Joint::REVOLUTE:
      joint.type = JointType::REVOLUTE_BOUNDED;
    ...
    case urdf::Joint::PRISMATIC:
      joint.type = JointType::PRISMATIC_BOUNDED;
      if ( urdf_joint->limits ) {
        joint.has_position_limits = true;
        joint.lower_limit = urdf_joint->limits->lower;
        joint.upper_limit = urdf_joint->limits->upper;
    ...
    if ( joint.has_position_limits && !( joint.lower_limit < joint.upper_limit ) ) {
      result.warnings.push_back( invalid_limits_warning( jn, joint.lower_limit, joint.upper_limit ) );

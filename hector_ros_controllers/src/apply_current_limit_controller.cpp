...
ApplyCurrentLimitController::command_interface_configuration() const
{
  ...
  // If we're following in a chain, don't claim HW command interfaces
  cfg.names = chained_mode_ ? std::vector<std::string>{} : command_interface_names_;
  return cfg;
}
...
ApplyCurrentLimitController::update_reference_from_subscribers(const rclcpp::Time & /*time*/,
...
ApplyCurrentLimitController::on_export_reference_interfaces()
{
  ...
  {
    refs.emplace_back(get_node()->get_name(), reference_interface_names_[i], &reference_interfaces_[i]);
  }
  return refs;
}
...
ApplyCurrentLimitController::on_export_state_interfaces()
{
  ...
  {
    states.emplace_back(get_node()->get_name(), exported_state_interface_names_[i], &state_interfaces_values_[i]);
  }
  return states;
}
...
ApplyCurrentLimitController::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  for (const std::string &type : command_interface_types_)
  {
    ...
    const bool ok = controller_interface::get_ordered_interfaces(
        command_interfaces_, joints_, type, ordered_cmd);
    if (!ok || ordered_cmd.size() != joints_.size())
    {
      RCLCPP_ERROR(this->get_node()->get_logger(),
                   "Expected %zu command interfaces for type '%s', got %zu",
                   joints_.size(), type.c_str(), ordered_cmd.size());
      return controller_interface::CallbackReturn::ERROR;
    }
  }
  ...
  for (const std::string &type : state_interface_types_)
  {
    ...
    const bool ok = controller_interface::get_ordered_interfaces(
        state_interfaces_, joints_, type, ordered_state);
    if (!ok || ordered_state.size() != joints_.size())
    {
      RCLCPP_ERROR(this->get_node()->get_logger(),
                   "Expected %zu state interfaces for type '%s', got %zu",
                   joints_.size(), type.c_str(), ordered_state.size());
      return controller_interface::CallbackReturn::ERROR;
    }
  }
                                                       ...
                                                       const rclcpp::Duration & /*period*/)
{
  ...
  if (!controller_interface::get_ordered_interfaces(command_interfaces_, joints_, "position", pos_cmd) ||
      pos_cmd.size() != joints_.size())
    return controller_interface::return_type::ERROR;
  if (!controller_interface::get_ordered_interfaces(command_interfaces_, joints_, "current", cur_cmd) ||
  ...
  return ok ? controller_interface::return_type::OK
